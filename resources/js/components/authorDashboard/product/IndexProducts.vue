  ...
  <div class="container mx-auto p-6">
    ...
    <div class="bg-white shadow rounded-lg overflow-hidden">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b">
        <!-- search Strat -->
         <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 px-4 py-4 border-b">
              
          <!-- Search Box -->
        <div class="relative w-full md:w-80">
          <div class="absolute inset-y-0 left-3 flex items-center pointer-events-none">
            <svg class="w-5 h-5 text-gray-400" viewBox="0 0 20 20" fill="none">
              <path d="M17.5 17.5L15.4167 15.4167M15.8333 9.16667C15.8333 5.48477..." stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>
            </svg>
          </div>
          <input
            type="text"
            placeholder="Search for company"
            class="w-full pl-10 pr-4 py-2 rounded-full border border-gray-300 focus:outline-none text-gray-900 placeholder-gray-400"
          />
        </div>
        ...
        <div class="flex flex-wrap gap-2 justify-end">
          <button @click="exportToExcel" class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600">
            Export Excel
          </button>
          <button @click="exportToCSV" class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">
            Export CSV
          </button>
          <button @click="exportToPDF" class="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600">
            Export PDF
          </button>
        </div>
        ...
        <table class="min-w-full table-auto text-sm text-left text-gray-700">
          <thead>
                        <tr class="bg-blue-600">
                          ...
                          <th scope="col" class="p-5 text-left text-sm leading-6 font-bold text-white capitalize rounded-t-xl"> ID </th>
                            
                          <th scope="col" class="p-5 text-left text-sm leading-6 font-bold text-white capitalize"> product Name </th>
                            
                          <th scope="col" class="p-5 text-left text-sm leading-6 font-bold text-white capitalize"> Product Code </th>

                          <th scope="col" class="p-5 text-left text-sm leading-6 font-bold text-white capitalize"> Buying Price </th>

                          <th scope="col" class="p-5 text-left text-sm leading-6 font-bold text-white capitalize"> Buying Date </th>

                          <th scope="col" class="p-5 text-left text-sm leading-6 font-bold text-white capitalize"> Selling Price </th>

                          <th scope="col" class="p-5 text-left text-sm leading-6 font-bold text-white capitalize"> Unit </th>

                          <th scope="col" class="p-5 text-left text-sm leading-6 font-bold text-white capitalize"> Stock Quantity </th>
                            
                          <th scope="col" class="p-5 text-left text-sm leading-6 font-bold text-white capitalize"> product Image </th>
                            
                          <th scope="col" class="p-5 text-left text-sm leading-6 font-bold text-white capitalize rounded-t-xl"> Actions </th>

                        </tr>
                    </thead>
          <tbody class="divide-y divide-gray-300 ">
                        <tr v-for="(product, index) in products" :key="product.id" class="bg-white transition-all duration-500 hover:bg-gray-50">
                            <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 "> {{ index + 1 }} </td>
                            
                            <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {{ product.products_name }} </td>
                            
                            <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {{ product.products_code }} </td>

                            <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {{ product.buying_price }} </td>

                            <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {{ product.buying_date }} </td>

                            <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {{ product.selling_price }} </td>

                            <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {{ product.unit }} </td>

                            <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {{ product.stock_quantity }} </td>

                            <td class="p-5 py-3">
                                <img :src="'/productImages/' + product.image" alt="Image" class="h-12 w-auto rounded shadow" />
                            </td>

                            <td>
                                <div class="flex items-center gap-1">
                                    
                                  <a :href="`/admin/product/show/${product.id}`" class="p-2 rounded-full  group transition-all duration-500  flex item-center">
                                  ...
                                  <a :href="`/admin/product/edit/${product.id}`" class="p-2  rounded-full  group transition-all duration-500  flex item-center">
                                    ...
                                    <button @click="deleteProduct(product.id)" class="p-2 rounded-full  group transition-all duration-500  flex item-center">
