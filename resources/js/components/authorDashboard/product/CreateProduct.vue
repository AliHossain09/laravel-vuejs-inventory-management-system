<template>
  <div class="container p-6 bg-white rounded shadow-md mx-auto mt-8 rounded-b-xl">
    <div class="flex items-center justify-between mb-4 border-b-2 shadow-md p-2 rounded-md">
      ...
      <h2 class="md:text-2xl sm:text-2xl xs:text-xl font-bold text-gray-700 flex items-center">
        <i class="fas fa-user mr-2"></i> Product Insert
      </h2>
    ...
    <!-- Form -->
    <form form @submit.prevent="saveProduct">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input v-model="form.buying_date" name="buying_date" type="date" placeholder="Buying date"
            class="w-full border border-gray-300 p-2 rounded" />
...
export default {
  ...
  if (data.success) {
    Swal.fire({
      icon: 'success',
      title: 'আপনার Product সফলভাবে তৈরি হয়েছে।',
      text: 'product সফলভাবে যুক্ত হয়েছে!',
      toast: true,
      position: 'top-end',
    ...
    axios.get('/api/dropdowns/product-form').then(response => {
      ...
      console.error("Dropdown data load failed", error);
    });
  }
